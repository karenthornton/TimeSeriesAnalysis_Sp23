...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)], frequency = 12, start = c(1931,1)) #adding the start will add the date (year) instead of just numbers
ts_inflow_data
```
...
#dummies = seasonaldummy(ts_inflow_data[t,iHP])
#lm(ts_inflow_data~dummies, inflow_data)
