...
wind_data <- read.csv(file="/Users/karenthornton/Documents/School/Grad School/Year 2/Semester 2/Time Series/TimeSeriesAnalysis_Sp23/Data/Wind_Speed_PortArthurTX_Processed.csv",header=TRUE,stringsAsFactors = TRUE)
...
decompose_wind_data=decompose(clean_wind_data,"additive")#type="multiplicative")
#stl instead of decompose (but this can only do additive), it gives a "remainder" category. smaller the bar the less pronounced it is
...
#wind_df <- data.frame("date"=wind_data$DATE,wind_random, wind_trend, wind_seasonal)

wind_df <- cbind(full_wind_data[,1], wind_random, wind_trend, wind_seasonal)
