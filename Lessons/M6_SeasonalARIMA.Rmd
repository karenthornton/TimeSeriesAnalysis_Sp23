...
Notes:
if middle are both 1 then you differentiate twice. this means there won't be any mu.

Steps to specify model orders
1. run staionary test
2.
3. plot ACF and PACF and look at the behavior of non-seasonal lags
4. check if seasonal differencing is needed
5. again ACF and PACF plots....
...
electricity_price <- read.csv(file="/Users/karenthornton/Documents/School/Grad School/Year 2/Semester 2/Time Series/TimeSeriesAnalysis_Sp23/Data/Average_retail_price_of_electricity_United_States_monthly.csv",header=TRUE,skip=4)
...
#lag is 12 because its seasonal
...
#this is the one that's been diferenced twice (which means no drift)
...
```{r}
#assuming seasonal is constant over time
#no seasonality incorporated here
...
Not really comparable. We took out the seasonal?? need to add the seasonal back to the last Arima and then compare to the rest of the. 