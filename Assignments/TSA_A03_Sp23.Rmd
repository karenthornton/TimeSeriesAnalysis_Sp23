...
for(i in 1:nenergy){
  ...
  pacf(ts_Renewable_Energy[,i],lag.max = 40,main=paste0("HP",i),ylim=c(-1,1))
}

#          print(ggplot(Renewable_Energy, aes(x=Month))+
        #geom_line(aes(y=name[i+1])))
...
linear_trend <- lm(Renewable_Energy[,iHP+1] ~ t)
...
```{r}
detrend_Renewable_Energy <- Renewable_Energy[,(iHP+1)]-(beta0+beta1*t)

ggplot(Renewable_Energy, aes(x=Month, y=Renewable_Energy[,iHP+1]))+
  geom_line(color="blue")+
  geom_line(aes(y=detrend_Renewable_Energy), color="green")
```
